...
module ProductTests
  ...
  def test_load
    user = PlaidLegacy::User.load(product, 't0k3n')

    assert_kind_of PlaidLegacy::User, user
    ...
    assert_raises ArgumentError do
      PlaidLegacy::User.load(:bad_product, 't0k3n')
    end
  end

  def test_load_custom_client
    client = PlaidLegacy::Client.new
    user = PlaidLegacy::User.load(product, 't0k3n', client: client)
  ...
  def test_for_product
    ...
    product2 = PlaidLegacy::PRODUCTS.reject { |p| p == product }.first
  ...
  def test_exchange_token
    ...
    user = PlaidLegacy::User.exchange_token('pu61i< T<>k3N',
                                            product: product)
  ...
  def test_exchange_token_with_account_id
    ...
    user = PlaidLegacy::User.exchange_token('pu61i< T<>k3N', 'abcdef',
                                            product: product)
  ...
  def test_question_mfa
    ...
    user = PlaidLegacy::User.create(product, :wells, 'plaid_test', 'plaid_good')
  ...
  def custom_client
    PlaidLegacy::Client.new(env: :tartan, client_id: 'boo', secret: 'moo')
  end
  ...
  def check_accounts(expected_count, accounts, &block)
    check_collection expected_count, accounts, PlaidLegacy::Account, &block
  end

  def check_transactions(expected_count, txs, &block)
    check_collection expected_count, txs, PlaidLegacy::Transaction, &block
  end

  def check_user(user)
    assert_instance_of PlaidLegacy::User, user
...
class PlaidConnectUserTest < MiniTest::Test
  ...
  def test_create_user_login_only
    ...
    user = PlaidLegacy::User.create(:connect, :wells, 'plaid_test', 'plaid_good',
  ...
  def test_transactions
    user = PlaidLegacy::User.load(:connect, 't0k3n')
...
class PlaidAuthUserTest < MiniTest::Test
  ...
  def test_create_user
    ...
    user = PlaidLegacy::User.create(:auth, :wells, 'plaid_test', 'plaid_good')
    check_user user
  ...
  def auth_user
    @auth_user ||= PlaidLegacy::User.load(:auth, 't0k3n')
  end
end
...
class PlaidInfoUserTest < MiniTest::Test
  ...
  def test_create_user
    ...
    user = PlaidLegacy::User.create(:info, :wells, 'plaid_test', 'plaid_good')
    check_user user
    check_accounts 4, user.accounts

    refute_nil user.info
    assert_kind_of PlaidLegacy::Info, user.info
  ...
  def info_user
    @info_user ||= PlaidLegacy::User.load(:info, 't0k3n')
  end
...
class PlaidIncomeUserTest < MiniTest::Test
  ...
  def test_create_user
    ...
    user = PlaidLegacy::User.create(:income, :wells, 'plaid_test', 'plaid_good')
    check_user user
    check_accounts 4, user.accounts

    refute_nil user.income
    assert_kind_of PlaidLegacy::Income, user.income
  ...
  def income_user
    @income_user ||= PlaidLegacy::User.load(:income, 't0k3n')
  end
...
class PlaidRiskUserTest < MiniTest::Test
  ...
  def test_create_user
    ...
    user = PlaidLegacy::User.create(:risk, :wells, 'plaid_test', 'plaid_good')
    check_user user
    check_accounts 4, user.accounts

    assert_kind_of PlaidLegacy::Risk, user.accounts[0].risk
  ...
  def risk_user
    @risk_user ||= PlaidLegacy::User.load(:risk, 't0k3n')
  end
